...
sns_plot = sns.pairplot(training_df, x_vars=["FARE", "TRIP_MILES", "TRIP_SECONDS"], y_vars=["FARE", "TRIP_MILES", "TRIP_SECONDS"])
...
def plot_model(df, features, weights, bias, fig):
    ...
    if len(features) == 1:
        ...
        for i in range(len(y)):
            x.append((z[i] - weights[1][0] * y[1] - bias[0]) / weights[0][0])

        plane=pd.DataFrame({'x':x, 'y':y, 'z':[z] * 3})

        light_yellow = [[0, '#89CFF0'], [1, '#FFDB58']]
        model = go.Figure(data=go.Surface(x=plane['x'], y=plane['y'],
        z=plane['z'], colorscale=light_yellow))
...
def model_info(feature_names, label_name, model_output):
    ...
    for index, feature in enumerate(feature_names):
        info = info + "Weight for feature[{}]: {:.3}\n".format(feature, weights[index][0])
...
def train_model(model, df, features, label, epochs, batch_size):
    """Train the model bu feeding it data."""
